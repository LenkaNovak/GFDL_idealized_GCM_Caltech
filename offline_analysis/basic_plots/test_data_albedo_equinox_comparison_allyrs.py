...
rctit = "1"
exp_name, run_name = ("uf_tests_hpc_businger_albedo_eqnx", "uf_tests_hpc_businger_albedo_eqnx_albedo02rcrit%s_13"%rctit)
save_basic_diags(exp_name, run_name, day_list, yr_range, save_variable_list, "data_rcrit%s/"%rctit, h5_name = "basic_vars")
...
plt.savefig("circ1_eqnx_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.savefig("circ1_eqnx_diff_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.savefig("circ2_eqnx_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.savefig("circ2_eqnx_diff_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))


# turbulent surface fluxes
plt.figure(figsize=(20,4))
zonal_plot(lon, lat, (np.nanmean(lh_flux_sfc,axis=0)), title = 'latent (W/m2)', subplot_no = (2,4,1) , vrange = np.arange(-50,51,5))
zonal_plot(lon, lat, np.nanmean(sh_flux_sfc,axis=0), title = 'sensible (W/m2)', subplot_no = (2,4,2) , vrange = np.arange(-50,51,5))
plt.subplot(2,4,3)
...
plt.subplot(2,4,5)
plt.hist(sh_flux_sfc[:,119:].flatten(), bins = np.arange(-100,100,1))
plt.ylabel('count')
plt.xlabel('sh_flux_sfc')
plt.ylim([0,0.02e6])
plt.subplot(2,4,6)
plt.hist(sh_flux_sfc_c[:,119:].flatten(), bins = np.arange(-100,100,1))
plt.hist((sh_flux_sfc)[:,119:].flatten(), bins = np.arange(-100,100,1), color = "r")
plt.ylim([0,0.02e6])
plt.xlabel('sh_flux_sfc_c')
plt.savefig("test_data_albedo_eqnx_comparison_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))

# differences of the above figures
plt.figure(figsize=(20,4))
zonal_plot(lon, lat, (np.nanmean(lh_flux_sfc,axis=0) - np.nanmean(lh_flux_sfc_c,axis=0)), title = 'latent (W/m2)', subplot_no = (1,5,1) )
zonal_plot(lon, lat, np.nanmean(sh_flux_sfc,axis=0) - np.nanmean(sh_flux_sfc_c,axis=0), title = 'sensible (W/m2)', subplot_no = (1,5,2) )
plt.subplot(1,5,3)
plt.plot(lat, (np.nanmean(lh_flux_sfc,axis=(0,-1)) - np.nanmean(lh_flux_sfc_c,axis=(0,-1))))
plt.title('latent (W/m2)')
plt.subplot(1,5,4)
plt.plot(lat, np.nanmean(sh_flux_sfc,axis=(0,-1)) - np.nanmean(sh_flux_sfc_c,axis=(0,-1)), label = "sensible")
plt.subplot(1,5,5)
plt.plot(lat, (np.nanmean(lh_flux_sfc,axis=(0,-1)) - np.nanmean(lh_flux_sfc_c,axis=(0,-1))), label = "latent")
plt.plot(lat, np.nanmean(sh_flux_sfc,axis=(0,-1)) - np.nanmean(sh_flux_sfc_c,axis=(0,-1)), label = "sensible")
plt.plot(lat, np.nanmean(lh_flux_sfc + sh_flux_sfc,axis=(0,-1)) - np.nanmean(lh_flux_sfc_c + sh_flux_sfc_c,axis=(0,-1)), label = "total")
plt.title('(W/m2)')
plt.legend()
plt.savefig("test_data_albedo_eqnx_comparison_diff_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.xlabel('ri_bulk')
# plt.ylim([0,0.2e6])
plt.savefig("hist_albedo_eqnx_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.xlabel('ri_bulk')
plt.ylim([0,0.02e6])
plt.savefig("hist_albedo_eqnx_polar_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
...
plt.xlabel('ri_bulk')
plt.ylim([0,0.02e6])
plt.savefig("hist_albedo_eqnx_polar_sh_allyrs_%s-%s.png"%(yr_range[0], yr_range[-1]))
