...
def get_phase_speed_spectra_v_latitude(u_02, v_02, day_list, lat, lon):
    ...
    for l4 in lat_sampled:
        ...
        u_lev = u_02[:,la_i,:]
        calculate_cospectra(v_lev, v_lev, lat, lon, t, dt_secs = 86400.0, fdir=".", fname="cospectra_lat_%00d_%s_hydro"%(l4, last_day_of_yr))
    nc = 60
    nm = int(len(lon) / 2)
    # get phase speed spectra
    K_wm = np.zeros((len(lat_sampled), int(2*nc), nm ))
    for i, l4 in enumerate(lat_sampled):
        K_wm[i, :, :], cp, m = phase_speed_spectra(l4, nc = nc, cmax = 40, fdir = ".", fname = "cospectra_lat_%00d_%s_hydro"%(l4, last_day_of_yr))
...
plt.savefig("ps_spectrum_hydro.png")
...
plt.plot(lat_c, np.mean(u_02_c, axis=(0,-1)))
plt.colorbar(c)
plt.savefig("ps_spectrum_diff_hydro.png")
