# exec(open("test_data_albedo_equinox_hydro_comparison.py").read())
...
# control run
exp_name, run_name = ("uf_tests_hpc_albedo_eqnx_hydro", "uf_tests_hpc_albedo_eqnx_hydro_albedo02_2")
...
lat_c, lon_c, sig_c, v_c, u_c, T_c, Th1_c, uv_c, vT_c, TT_c, eke_c, u_vrtcl_flux_c, pot_temp_vrtcl_flux_c, z1_c, T1_c, u1_c, lh_flux_sfc_c, sh_flux_sfc_c, T_sfc_c = append_year_days_zonal_mean(ppp_dir+fname%(last_day_of_yr, "%s"), day_list)

# control run
exp_name, run_name = ("uf_tests_hpc_businger_albedo_eqnx_hydro", "uf_tests_hpc_businger_albedo_eqnx_hydro_albedo02rcrit8_0")
work_dir = "/central/scratch/elencz/caltech_fms_idealized/%s/%s/workdir"%(exp_name, run_name)
output_dir = "/central/scratch/elencz/caltech_fms_idealized/%s/%s/output"%(exp_name, run_name)
ppp_dir = output_dir + "/ppp_output/"
fname = "day%sh00.segment%s.nc"
lat, lon, sig, v, u, T, Th1, uv, vT, TT, eke, u_vrtcl_flux, pot_temp_vrtcl_flux, z1, T1, u1, lh_flux_sfc, sh_flux_sfc, T_sfc = append_year_days_zonal_mean(ppp_dir+fname%(last_day_of_yr, "%s"), day_list)
...
plt.savefig("circ1_eqnx_hydro.png")
...
plt.savefig("circ1_eqnx_hydro_diff.png")
...
plt.savefig("circ2_eqnx_hydro.png")
...
plt.savefig("circ2_eqnx_hydro_diff.png")
...
plt.savefig("test_data_albedo_eqnx_hydro_comparison.png")
...
plt.title('sensible (W/m2)')
plt.savefig("test_data_albedo_eqnx_hydro_comparison_diff.png")
...
plt.xlabel('ri_bulk')
# plt.ylim([0,0.2e6])
plt.savefig("hist_albedo_eqnx_hydro.png")
...
plt.xlabel('ri_bulk')
plt.ylim([0,0.02e6])
plt.savefig("hist_albedo_eqnx_hydro_polar.png")
...
plt.xlabel('ri_bulk')
plt.ylim([0,0.02e6])
plt.savefig("hist_albedo_eqnx_hydro_polar_sh.png")
